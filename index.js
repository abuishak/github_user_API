...
function displayResults(responseJson) {
    ...
    let userinfo = `
        ...
        <h4><span class="user">Repos: ${responseJson.length}</span></h3>
    ...
    for (let i = 0; i < responseJson.length; i++) {
        $('.js-results').append(`
        <div class="result-item"><li><h4>${responseJson[i].name}</h3>
...
function getSearchPhrase() {
    return (['Search', 'Find', 'Gitty up GitHub','Look up All Result','Go','Push this button','Don\'t push this button'])[Math.floor(Math.random() * 8)];
}
